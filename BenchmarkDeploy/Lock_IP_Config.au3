...
Func _Main()
	...
	$sLogPath = "C:\BenchmarkTest\BenchmarkTest.log"
	
	_FileWriteLog($sLogPath, "-------锁定IP配置信息开始-------")
	...
	If Ping("www.baidu.com") > 0 Then
		...
		;成功后创建完成文件标识
		If _FileCreate("C:\BenchmarkTest\LockSuccess.Mark") = 1 Then
			_FileWriteLog($sLogPath, "成功;联网成功后创建完成文件标识成功")
		Else
			_FileWriteLog($sLogPath, "失败;联网成功后创建完成文件标识失败")
		EndIf
	Else
